...
export const constructPathAStar = (
  ...
  openSet.add({
    cell: start,
    f: costSoFar[start.y][start.x] + calculateHeuristic(start, exits),
    predecessor: null,
  });
  ...
  while (!openSet.isEmpty()) {
    ...
    for (const neighbour of emptyNeighbours) {
      ...
      if (g < costSoFar[neighbour.y][neighbour.x]) {
        ...
        openSet.add({
          cell: neighbour,
          f: g + calculateHeuristic(neighbour, exits),
          predecessor: closest,
        });
      }
    }
...
const calculateHeuristic = (a: CellCoords, exits: CellCoords[]): number => {
